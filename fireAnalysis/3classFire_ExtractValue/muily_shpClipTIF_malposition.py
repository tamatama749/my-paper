...
# ------------------ 配置区：根据实际情况修改路径------------------
# 1. shp矢量文件路径
shp_path = r'E:/CA_Fire_Analysis/everyYrWUIburnSHP/allYr_M_Interface.shp'
...
# 4. 选择剪裁模式：'next' 表示用当前年份shp剪裁下一年tif，'prev' 表示用当前年份shp剪裁上一年tif
# ###############################################################################################################
clip_mode = 'next'  # 或 改为'prev'
# ###############################################################################################################
# 确保输出文件夹存在
        ...
        try:
            ...
            if clip_mode == 'next':
                ...
                if i > 0:
                    ...
                    print(f'没有上一年的tif可供{year}年剪裁')
                    continue
            
            # 查找对应目标年份的.tif文件
            tif_pattern = os.path.join(tif_folder, f'CStocks_Live_{target_year}.tif')
            
            if not os.path.exists(tif_pattern):
                print(f'缺少目标年份的tif文件：{tif_pattern}')
            ...
            # 输出
            # ##############################################################################################################
            out_fp = os.path.join(output_folder, f'AGB{year}_clip_{target_year}.tif')
            # ##############################################################################################################
