...
# list(colormaps)
# # 读取数据
# fire_class1 = pd.read_csv('E:/CA_Fire_Analysis/deepAnaly/cause_natural.csv', encoding='utf-8')
...
#                       s=100, c=yearly_stats['total_area'], cmap='gist_rainbow', edgecolors='gray', alpha=0.7)
...
# plt.fill_between(x_line, y_line - y_err, y_line + y_err, color='#DAA520', alpha=0.1, edgecolor='none', label="95% Confidence Interval")
...
# 创建一个颜色列表以区分不同的文件
colors = ['#4169E1','#8B4513','#DAA520', '#363636']
# 创建火灾等级对应的图例标签
...
for i, file_path in enumerate(file_paths):
    ...
    fire_stats = fire_stats.groupby('YEAR').agg(
        ...
        avg_elevation=('DEM', 'mean')  # 计算该年所有火灾的平均海拔
    ).reset_index()
    ...
    # 三次多项式回归拟合
    z = np.polyfit(fire_stats['YEAR'], fire_stats['avg_elevation'], 2)
    p = np.poly1d(z)
...
# 设置标题、标签和图例
plt.title('Fire Average Elevation Over Time by 4 Causes', fontsize=22)
plt.xlabel('Year', fontsize=18)
plt.ylabel('Average Elevation (m)', fontsize=18)
plt.legend(bbox_to_anchor=(1.05, 0), loc=3, borderaxespad=0,title='Fire Classes')
